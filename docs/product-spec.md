...
- プラン別にメンバー上限あり（Small=5人、Standard=15人、Business=40人）
...
### 料金（2026-02-15確定）
- **Compass Small（〜5名）**: ¥5,000/月（社単位）
- **Compass Standard（〜15名）**: ¥15,000/月（社単位）
- **Compass Business（〜40名）**: ¥35,000〜/月（社単位、将来Gen連携・AI後）
- **Compass Enterprise（100名〜）**: 要相談
- **サークル**: 既存料金（3席込み）
- **初期費用**: ¥0
- **無料トライアル**: 14日間
- **注**: 旧モデル「席単価¥1,000」は廃止、社単位ティア制に移行
...
### 3-1. 基本ルール
- 各組織に `seatLimit`（契約メンバー上限）がある
- アクティブユーザー数が `seatLimit` を超えられない
- 超えそうになったらプランアップグレード導線を表示

### 3-2. メンバー上限の決まり方

| パターン | seatLimit | seatSource |
|---------|-----------|------------|
| トライアル中 | 無制限（or 5名） | `trial` |
| サークル会員 | 3名 | `circle` |
| Compass単独 | 購入メンバー上限 | `subscription` |
| サークル + 追加枠 | 3 + 追加分 | `circle` |

### 3-3. サークル特典（組織単位で3名固定）

**ルール**:
- 組織内にサークル会員が1人でもいれば3名
- 会員が複数いても3名のまま（増殖しない）
- 4名目以降はCompass有料プランへアップグレード

**理由**:
1. 判定ロジックがシンプル
2. 「メンバー枠を増やしたい=有料プラン」という導線が明確
3. サークルは「学び」が価値であり、メンバー枠で釣らない
...
2. デモを触る（ログイン不要）
3. 14日トライアル開始（組織作成）
4. 価格（Small ¥5,000/月、Standard ¥15,000/月、社単位）
...
| Compass単独購入 | **実装済み**: checkout.ts（デプロイ待ち） | 可能（Small: ¥5,000/月〜） |
